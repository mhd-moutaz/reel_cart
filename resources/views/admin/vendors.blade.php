    ...
    <div class="addresses-grid users-grid">
        @foreach ($users as $u)
            <div class="address-card user-card" 
                data-user-id="{{ $u->id }}"
                data-name="{{ strtolower($u->user->name ?? '') }}" 
                data-email="{{ strtolower($u->user->email ?? '') }}"
                data-phone="{{ $u->user->phone_number ?? '' }}"
                data-work-location="{{ strtolower($u->has_store ?? '') }}"
                data-verification="{{ strtolower($u->verification_status ?? 'pending') }}">

                <div class="address-header user-header">
                    <div class="address-icon avatar-wrapper">
                        @if (!empty($u->user->avatar))
                            <img src="{{ asset('storage/' . $u->user->avatar) }}" alt="{{ $u->user->name }}" class="avatar-img">
                        @else
                            <div class="avatar-fallback">{{ strtoupper(substr($u->user->name ?? '-', 0, 1)) }}</div>
                        @endif
                    </div>
                ...
                <div class="address-details user-details">
                    ...
                    <div class="address-row">
                        ...
                        <span class="address-value">{{ $u->national_id ?? '-' }}</span>
                    </div>
                    
                    <div class="address-row">
                        <span class="address-label">نوع العمل:</span>
                        <span class="address-value">{{ $u->business_type ?? '-' }}</span>
                    </div>
                    
                    <div class="address-row">
                        <span class="address-label">مكان العمل:</span>
                        <span class="work-location-badge {{ strtolower($u->has_store ?? '') }}">
                            {{ $u->has_store == 'online' ? '🌐 Online' : '🏪 In Site' }}
                        </span>
                    </div>
                    
                    <div class="address-row">
                        <span class="address-label">اسم المحل:</span>
                        <span class="address-value">{{ $u->store->store_name ?? '-' }}</span>
                    </div>
                    
                    <div class="address-row">
                        <span class="address-label">حالة التحقق:</span>
                        <span class="verification-status {{ strtolower($u->verification_status ?? 'pending') }}">
                            @php
                                $statusText = [
                    ...
                    <div class="verification-toggle">
                        ...
                        <select class="verification-select" data-user-id="{{ $u->id }}">
                            <option value="pending" {{ strtolower($u->verification_status ?? 'pending') == 'pending' ? 'selected' : '' }}>
                                قيد الانتظار
                            </option>
                            <option value="verified" {{ strtolower($u->verification_status ?? '') == 'verified' ? 'selected' : '' }}>
                                موثق
                            </option>
                            <option value="rejected" {{ strtolower($u->verification_status ?? '') == 'rejected' ? 'selected' : '' }}>
                                مرفوض
                            </option>
                        </select>
                        <button class="update-btn" onclick="updateVerification({{ $u->id }})">
                            تحديث
                        </button>
                    </div>
                </div>
